...
import { Employee, EmployeeAdvance, EmployeeAttendance, EmployeeDeductionConfig } from "../types";
...
type SubTabType = "directory" | "attendance" | "advances" | "inquiry" | "rules";
...
export default function EmployeesTab({ onShowToast, userRole }: EmployeesTabProps) {
  ...
  const loadAllData = async () => {
    setLoading(true);
    try {
      const [empRes, advRes, attRes, configRes] = await Promise.all([
        fetch("/api/employees"),
  ...
  const handleSaveAdvance = async (e: React.FormEvent) => {
    ...
    // Calculate maximum allowed borrow percentage
    const maxBorrowAmount = (emp.salary * emp.advanceLimitPercent) / 100;
    if (advAmount > maxBorrowAmount) {
      onShowToast(
        `⚠️ تجاوز السقف: الحد الأقصى للسلف لهذا الموظف هو ${emp.advanceLimitPercent}% من الراتب (${maxBorrowAmount.toFixed(
          2
  ...
  const getInquiryReport = () => {
    ...
    return reportList.map(emp => {
      ...
      const totalDeductions = empAtt.reduce((sum, item) => sum + item.deductionAmount, 0);
              ...
              {employees.length === 0 ? (
                ...
                <form onSubmit={handleSaveAttendance} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="text"
                      placeholder="حالة حضور طبيعية، غياب مبرر، اعتذار صحي"
                      value={attNotes}
              ...
              <div className="flex flex-wrap items-center justify-between border-b border-slate-100 pb-3 gap-2">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {userRole === "مدير" && employees.length > 0 && (
                    <button
                      onClick={() => {
                        // Gather detailed elements for all employees in current scope
                  ...
                  <table className="w-full text-right border-collapse">
                    <thead>
                      <tr className="border-b border-slate-100 text-slate-500 text-[10px] font-bold uppercase">
                        ...
                        <th className="py-3 px-2 text-center">خصميات التأخير</th>
                    ...
                    <tbody className="divide-y divide-slate-50 text-xs">
                      {reportData.map((emp) => (
                        <tr key={emp.id} className="hover:bg-slate-50/60 font-medium">
                          ...
                          <td className="py-3.5 px-2 text-center font-bold text-rose-700 font-mono">
                            {emp.totalDeductions > 0 ? `${emp.totalDeductions.toLocaleString()} ر.س` : "0"}
                          </td>
              ...
              {printEmployeeData.data.map((item: any, idx: number) => (
                <div key={idx} className="print-item border-b border-slate-300 pb-10 last:border-0 last:pb-0 font-sans text-xs leading-relaxed" style={{ pageBreakAfter: printEmployeeData.data.length > 1 ? "always" : "auto" }}>
                  ...
                  <div className="my-6 bg-slate-50 p-4 rounded-2xl border border-slate-100 grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      <span className="block text-[10px] text-slate-400 font-bold">سقف الاقتراض المسموح:</span>
                      <strong className="text-indigo-800 text-xs font-mono">{item.employee.advanceLimitPercent || 25}% ({((item.employee.salary * (item.employee.advanceLimitPercent || 25)) / 100).toLocaleString("ar-SA")} ر.س)</strong>
                    </div>
                  </div>
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {item.attendance.map((att: any, attIdx: number) => (
                            <tr key={attIdx} className="hover:bg-slate-50/50">
                              ...
                              <td className="p-2 border border-slate-200 text-slate-500 font-semibold">{att.notes || "حضور طبيعي في المخطط"}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div className="mt-8 bg-slate-100 p-5 rounded-2xl border border-slate-200/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-1">
                      ...
                      <div className="text-[11px] text-slate-600 font-semibold leading-relaxed">
                        ...
                        (مقسمة تفصيلاً إلى: <span className="font-mono text-xs text-rose-700">{(item.totals.totalDeductions || 0).toLocaleString("ar-SA")} ر.س</span> غرامات تأخر ، 
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-200 grid grid-cols-3 gap-6 text-center text-[10px] font-bold text-slate-600">
                    <div className="space-y-10">
                      <span>توقيع واعتماد المدير العام للشركة:</span>
                      <div className="border-b border-dashed border-slate-400 w-3/4 mx-auto pb-1 mt-4 text-slate-400 text-[9px]">الاسم:................................................</div>
                    </div>
                    <div className="space-y-10">
                      <span>توقيع وموافقة المحاسب المالي للفرع:</span>
                      <div className="border-b border-dashed border-slate-400 w-3/4 mx-auto pb-1 mt-4 text-slate-400 text-[9px]">الاسم:................................................</div>
                    </div>
                    <div className="space-y-10">
                      <span>بصمة وإمضاء الموظف المستحق للاستلام:</span>
                      <div className="border-b border-dashed border-slate-400 w-3/4 mx-auto pb-1 mt-4 text-slate-400 text-[9px]">توقيع:................................................</div>
                    </div>
                  </div>
