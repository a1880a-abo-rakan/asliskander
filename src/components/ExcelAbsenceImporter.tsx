...
export default function ExcelAbsenceImporter({
  ...
  // Persistent imported records in localStorage across section navigation
  const [importedRecords, setImportedRecords] = useState<ImportedAbsenceRecord[]>(() => {
    try {
      const saved = localStorage.getItem("pending_excel_absence_records");
      if (saved) {
      ...
      console.error("Failed to load saved imported records", e);
    }
    return [];
  });
  ...
  const [fileName, setFileName] = useState<string>(() => {
    try {
      return localStorage.getItem("pending_excel_absence_filename") || "";
    } catch {
      return "";
    }
  });

  // Sync to localStorage whenever importedRecords or fileName changes
  useEffect(() => {
    try {
      if (importedRecords.length > 0) {
        localStorage.setItem("pending_excel_absence_records", JSON.stringify(importedRecords));
        if (fileName) {
          localStorage.setItem("pending_excel_absence_filename", fileName);
        }
      } else {
        localStorage.removeItem("pending_excel_absence_records");
        localStorage.removeItem("pending_excel_absence_filename");
      }
    } catch (e) {
      console.error("Failed to save pending imported records", e);
    }
  }, [importedRecords, fileName]);
  ...
  const [selectedRecordIds, setSelectedRecordIds] = useState<string[]>(() => {
    try {
      ...
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed)) return parsed.map((r: any) => r.id);
      }
    } catch {}
    return [];
  });
    ...
    setAttendanceRecords((prev) => {
      ...
      records.forEach((record) => {
        ...
        dayData[studentKey] = {
          ...
          notes: "", // Notes left empty as requested
  ...
  // Delete all imported records if user does not wish to send them
  const handleDeleteAllImportedRecords = () => {
    if (importedRecords.length === 0) return;
    if (window.confirm("هل أنت متأكد من حذف جميع الأسماء المستوردة من إكسل وإلغاء الاستيراد؟")) {
      // Clean up un-notified records from attendanceRecords for this date
      const recordKeys = new Set<string>(importedRecords.map((r) => r.matchedStudentId || r.id));
      setAttendanceRecords((prev) => {
        if (!prev[selectedDate]) return prev;
        const day = { ...prev[selectedDate] };
        let modified = false;
        recordKeys.forEach((k: string) => {
          if (day[k] && !day[k].notified) {
            delete day[k];
            modified = true;
          }
        });
      ...
      showToastMsg("✓ تم حذف جميع الأسماء المستوردة بنجاح");
    }
  };

  // Delete selected imported records
  const handleDeleteSelectedRecords = () => {
    if (selectedRecordIds.length === 0) return;
    if (window.confirm(`هل أنت متأكد من حذف (${selectedRecordIds.length}) من الأسماء المستوردة المحددة؟`)) {
      const toDelete = importedRecords.filter((r) => selectedRecordIds.includes(r.id));
      const recordKeys = new Set<string>(toDelete.map((r) => r.matchedStudentId || r.id));
      ...
      setImportedRecords((prev) => prev.filter((r) => !selectedRecordIds.includes(r.id)));
      setSelectedRecordIds([]);
      showToastMsg("✓ تم حذف الأسماء المحددة بنجاح");
    }
  };

  // Delete single imported record
  const handleDeleteSingleRecord = (record: ImportedAbsenceRecord) => {
    if (window.confirm(`هل أنت متأكد من حذف الطالب (${record.effectiveName}) من القائمة المستوردة؟`)) {
      const studentKey = record.matchedStudentId || record.id;
      setAttendanceRecords((prev) => {
        if (!prev[selectedDate] || !prev[selectedDate][studentKey]) return prev;
        const day = { ...prev[selectedDate] };
        if (!day[studentKey].notified) {
          delete day[studentKey];
          const next = { ...prev, [selectedDate]: day };
          try {
            localStorage.setItem("school_attendance_records", JSON.stringify(next));
            saveAttendanceDataToCloud(next).catch(console.error);
      ...
      setImportedRecords((prev) => prev.filter((r) => r.id !== record.id));
      setSelectedRecordIds((prev) => prev.filter((id) => id !== record.id));
      showToastMsg(`✓ تم حذف الطالب (${record.effectiveName}) من قائمة الاستيراد`);
    }
  };
  ...
  const handleSendSingleRecord = async (record: ImportedAbsenceRecord, force = false) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Remove from importedRecords so it disappears from the import section after being sent
        setImportedRecords((prev) => prev.filter((r) => r.id !== record.id));
        setSelectedRecordIds((prev) => prev.filter((id) => id !== record.id));
    ...
    for (let i = 0; i < batchCandidates.length; i++) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // Automatically unselect notified record and remove from pending imported records
          setSelectedRecordIds((prev) => prev.filter((id) => id !== record.id));
          setImportedRecords((prev) => prev.filter((r) => r.id !== record.id));
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {importedRecords.length > 0 && (
            <button
              type="button"
              onClick={handleDeleteAllImportedRecords}
              className="px-3 py-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer border border-rose-200"
              title="حذف جميع الأسماء المستوردة من إكسل في حال عدم الرغبة بالإرسال"
            >
              <Trash2 className="w-3.5 h-3.5" />
              <span>حذف الأسماء المستوردة</span>
            </button>
          )}
        </div>
      </div>
              ...
              <div className="flex flex-wrap items-center gap-2.5">
                ...
                {selectedRecordIds.length > 0 && (
                  <button
                    ...
                    className="px-3.5 py-2.5 rounded-xl bg-rose-900/60 hover:bg-rose-800 text-rose-200 font-bold text-xs flex items-center gap-1.5 cursor-pointer transition-all border border-rose-600/50 shadow-xs"
                    title="حذف الطلاب المحددين من قائمة الاستيراد"
                  >
                    <Trash2 className="w-3.5 h-3.5 text-rose-300" />
                    <span>حذف المحدد ({selectedRecordIds.length})</span>
                  </button>
                )}
                        ...
                        >
                          ...
                          <td className="p-3 text-center">
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              {/* Delete Record from Import List */}
                              <button
                                type="button"
                                onClick={() => handleDeleteSingleRecord(record)}
                                className="p-1.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-600 transition-all cursor-pointer border border-rose-200"
                                title="حذف هذا الطالب من قائمة الاستيراد"
                              >
                                <Trash2 className="w-3.5 h-3.5" />
                              </button>
