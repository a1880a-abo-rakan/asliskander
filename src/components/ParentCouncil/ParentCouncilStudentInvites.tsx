...
export default function ParentCouncilStudentInvites({
  ...
  const buildInviteUrl = (token: string, code: string, student?: Student) => {
    const origin = typeof window !== "undefined" ? window.location.origin : "";
    const params = new URLSearchParams();
    params.set("portal", "parent-council");
    params.set("parent_council", "true");
    params.set("council_token", token);
    params.set("token", token);
    params.set("code", code);
    params.set("council_code", code);

    if (student) {
      if (student.id) params.set("student_id", String(student.id));
      const sName = student.name || (student as any)["اسم الطالب"] || "";
      if (sName) params.set("student_name", sName);
      const sGrade = student.grade || (student as any)["الصف"] || "";
      if (sGrade) params.set("grade", sGrade);
      ...
      if (sPhone) params.set("phone", sPhone);
    }
  ...
  const buildStudentMessage = (student: Student, invite: ParentCouncilInvite) => {
    ...
    const sGrade = student.grade || (student as any)["الصف"] || "";
    const sClass = student.className || (student as any)["الفصل"] || (student as any)["الشعبة"] || "";
    const url = buildInviteUrl(invite.token, invite.code, student);
                    ...
                    >
                      ...
                      <td className="p-3 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <a
                            href={buildInviteUrl(invite.token, invite.code, student)}
                            target="_blank"
                            rel="noreferrer"
