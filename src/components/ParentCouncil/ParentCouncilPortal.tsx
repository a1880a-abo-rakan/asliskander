...
export default function ParentCouncilPortal({
  ...
  useEffect(() => {
    ...
    // 4. If token has student ID embedded (pc_<id>_<hash>), match against loaded roster
    const studentIdFromToken =
      token && (token.startsWith("pc_") || token.startsWith("council_"))
        ? token.split("_")[1]
        : null;
    if (studentIdFromToken && loadedStudents.length > 0) {
      const matched = loadedStudents.find(
        (s) =>
          String(s.id) === String(studentIdFromToken) ||
          (s as any)["رقم الطالب"] === String(studentIdFromToken)
      );
      if (matched) {
        const sName = matched.name || (matched as any)["اسم الطالب"] || "";
        const sGrade = matched.grade || (matched as any)["الصف"] || "الأول ثانوي";
        const sClass =
          matched.className || (matched as any)["الفصل"] || (matched as any)["الشعبة"] || "1";
        const sPhone = matched.phone || (matched as any)["رقم الجوال"] || "";

        setStudentName((prev) => prev || sName);
        setStudentGrade((prev) => prev || sGrade);
        setStudentClass((prev) => prev || sClass);
        setStudentId((prev) => prev || String(matched.id || ""));
        if (sPhone) setPhone((prev) => prev || sPhone);

        const derivedGuardian = extractGuardianFullName(matched);
        if (derivedGuardian) setFullName((prev) => prev || derivedGuardian);

        const siblings = findStudentSiblings(matched, loadedStudents);
        if (siblings.length > 0) setDetectedSiblings(siblings);
      }
    }

    // 5. Fetch server data for this token if available
    if (token) {
      fetch(`/api/parent-councils/token/${token}`)
        ...
        .then((data) => {
          ...
          // Also populate from invite details if available
          if (data?.invite) {
            const inv = data.invite;
            if (inv.studentName) setStudentName((prev) => prev || inv.studentName);
            if (inv.studentId) setStudentId((prev) => prev || String(inv.studentId));
            if (inv.studentGrade) setStudentGrade((prev) => prev || inv.studentGrade);
            if (inv.studentClass) setStudentClass((prev) => prev || inv.studentClass);
            if (inv.guardianPhone) setPhone((prev) => prev || inv.guardianPhone);
            if (inv.code) {
              setActivationCodeInput((prev) => prev || inv.code);
              setIsCodeVerified(true);
            }
          }

          // Populate from server matched student
          if (data?.student) {
            const st = data.student;
            const sName = st.name || st["اسم الطالب"] || "";
            const sGrade = st.grade || st["الصف"] || "";
            const sClass = st.className || st["الفصل"] || st["الشعبة"] || "";
            const sPhone = st.phone || st["رقم الجوال"] || "";
            if (sName) setStudentName((prev) => prev || sName);
            if (st.id) setStudentId((prev) => prev || String(st.id));
            if (sGrade) setStudentGrade((prev) => prev || sGrade);
            if (sClass) setStudentClass((prev) => prev || sClass);
            if (sPhone) setPhone((prev) => prev || sPhone);
            const derivedGuardian = extractGuardianFullName(st);
            if (derivedGuardian) setFullName((prev) => prev || derivedGuardian);
          }
        })
        .catch(() => {});
  ...
  const handleVerifyCode = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/parent-councils/verify-code", {
        ...
        body: JSON.stringify({ code: cleanCode, token, studentId }),
      });
