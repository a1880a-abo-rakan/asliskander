        ...
        if (!rosterStudentIds.has(String(extraKey)) && !map[extraKey]) {
          ...
          if (rec && rec.status && rec.status !== "present") {
            ...
            map[extraKey] = {
              ...
              lastSentMessage: lastMsg || "إشعار غياب",
            };
          }
        }
      });
    ...
    targetDates.forEach((dStr) => {
      ...
      Object.keys(dayData).forEach((extraKey) => {
        if (!studentIdsInRoster.has(String(extraKey))) {
          const rec = dayData[extraKey] as any;
          if (rec && rec.status && rec.status !== "present") {
            // Clean notes: do not mention Excel import as requested by user
            const rawNote = rec.notes || "";
            const cleanNote = (rawNote.includes("إكسل") || rawNote.includes("اكسل")) ? "" : rawNote;

            items.push({
              id: `${dStr}_${extraKey}`,
              studentId: extraKey,
              studentName: rec.customName || rec.name || "طالب",
              civilId: rec.civilId || "",
                    ...
                    return (
                      <tr key={item.id || index} className="hover:bg-slate-50/80 transition-colors print-avoid-break">
                        ...
                        <td className={`${getRowPaddingClass()} text-slate-700 border-l border-slate-200 text-[10px] align-top`}>
                          {(item.notes && !item.notes.includes("إكسل") && !item.notes.includes("اكسل")) 
                            ? item.notes 
                            : (isTardy ? `تأخر ${item.tardyMinutes || 15} دقيقة عن الاصطفاف` : "")}
                        </td>
