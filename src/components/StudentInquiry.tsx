...
export interface InquiryBatchProgress {
  isOpen: boolean;
  isRunning: boolean;
  isPaused: boolean;
  isCompleted: boolean;
...
export default function StudentInquiry({
  ...
  // Anti-Ban Variable Jitter Interval Sending State
  const [inquiryBatchProgress, setInquiryBatchProgress] = useState<InquiryBatchProgress>({
    isOpen: false,
    isRunning: false,
    isPaused: false,
    isCompleted: false,
  ...
  const abortInquiryBatchRef = useRef<boolean>(false);
  const pauseInquiryBatchRef = useRef<boolean>(false);
  ...
  // Dispatch Inquiries with Anti-Ban Variable Jitter Interval
  const handleSendInquiries = async () => {
    ...
    abortInquiryBatchRef.current = false;
    pauseInquiryBatchRef.current = false;

    setInquiryBatchProgress({
      isOpen: true,
      isRunning: true,
      isPaused: false,
      isCompleted: false,
      currentIndex: 0,
      total: requestsPayload.length,
      sentCount: 0,
      failedCount: 0,
      currentTeacherName: requestsPayload[0]?.teacherName || "",
      currentSubject: requestsPayload[0]?.subject || "",
      currentSection: requestsPayload[0]?.section || "",
      currentPhone: requestsPayload[0]?.teacherPhone || "",
      currentStudentsCount: selectedStudentsList.length,
      countdownSeconds: 0,
      logs: [],
    });
    ...
    let accumulatedInquiries = [...inquiryRequests];
    let sentSuccessCount = 0;

    try {
      for (let i = 0; i < requestsPayload.length; i++) {
        if (abortInquiryBatchRef.current) break;

        // Wait while paused
        while (pauseInquiryBatchRef.current) {
          if (abortInquiryBatchRef.current) break;
          await new Promise((r) => setTimeout(r, 500));
        }
        if (abortInquiryBatchRef.current) break;

        const currentReq = requestsPayload[i];

        setInquiryBatchProgress((prev) => ({
          ...prev,
          currentIndex: i + 1,
          currentTeacherName: currentReq.teacherName,
          currentSubject: currentReq.subject,
          currentSection: currentReq.section,
          currentPhone: currentReq.teacherPhone || "",
          countdownSeconds: 0,
        }));
        ...
        try {
          ...
          if (!res.ok) {
            throw new Error(data.error || "فشل إرسال الاستعلام");
          }

          if (data.inquiries && Array.isArray(data.inquiries) && data.inquiries.length > 0) {
            accumulatedInquiries = [...data.inquiries, ...accumulatedInquiries];
            onUpdateInquiries(accumulatedInquiries);
          }
          ...
          setInquiryBatchProgress((prev) => ({
            ...
            logs: [
              {
                teacherName: currentReq.teacherName,
                subject: currentReq.subject,
                phone: currentReq.teacherPhone || "غير مسجل",
                status: "success",
                timestamp: nowStr,
              },
              ...prev.logs,
            ...
            logs: [
              {
                ...
                phone: currentReq.teacherPhone || "غير مسجل",
                status: "failed",
                error: itemErr.message || "تعذر إرسال الرسالة",
                timestamp: nowStr,
              },
              ...prev.logs,
            ],
        ...
        // Safe 15-second Anti-Ban Delay with human-like jitter variation between messages (except after last message)
        if (i < requestsPayload.length - 1 && !abortInquiryBatchRef.current) {
          // 15 seconds base + random jitter (-1 to +2 seconds), total 14s to 18s
          const jitterSecs = Math.floor(Math.random() * 4) - 1;
      ...
      if (sentSuccessCount > 0) {
        setInquirySuccessBanner(
          `تم إرسال ${sentSuccessCount} طلب استعلام بنجاح إلى المعلمين المحددين عبر واتساب بنظام الفاصل الزمني والتفاوت البشري.`
        );
        setSelectedStudentIds([]);
        setCandidateTeachers([]);
        setSelectedTeacherKeys([]);
  ...
  return (
    <div className="space-y-6" dir="rtl">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-emerald-100/80">
                    يتم الإرسال بفاصل أمان (15 ثانية) مع تفاوت بشري عشوائي لمنع الحظر وحماية رقم المدرسة.
                  </span>
                </div>
              </div>

              <div className="px-3 py-1 rounded-xl bg-emerald-800/60 border border-emerald-600/40 text-emerald-200 text-[10px] font-bold shrink-0 hidden sm:flex items-center gap-1.5">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
                <span>نظام الإرسال الآمن المتتالي</span>
              </div>
            </div>
      ...
      {/* ========================================================================= */}
      {/* MODAL: VIEW INQUIRY DETAILS & OFFICIAL REPORT (A4, 1.15cm margin, ink-saver) */}
      {/* ========================================================================= */}
      {viewingInquiryModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs print:static print:p-0 print:bg-white">
          {/* Print Stylesheet for single inquiry report */}
          <style>{`
            @media print {
              @page {
                size: A4 portrait;
          ...
          <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 space-y-6 shadow-2xl border border-slate-200 print:max-h-none print:overflow-visible print:border-none print:shadow-none print:p-0">
            ...
            {/* Official printable report paper - Ink-Saving Optimized with crisp clean lines */}
            <div className="p-6 bg-white rounded-2xl border border-slate-300 space-y-5 printable-inquiry-paper" id="printable-inquiry-report">
              
              {/* Report Header */}
              <div className="text-center pb-4 border-b-2 border-slate-800 space-y-1">
                <span className="text-xs text-slate-600 block">المملكة العربية السعودية • وزارة التعليم</span>
              ...
              {/* Inquiry Meta Grid (Ink Saver: Clean white with 1px black/slate border) */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                <div className="bg-white p-2.5 rounded-lg border border-slate-300">
                  <span className="text-slate-500 block text-[10px]">المعلم:</span>
                ...
                <div className="bg-white p-2.5 rounded-lg border border-slate-300">
                  <span className="text-slate-500 block text-[10px]">الشعبة:</span>
              ...
              <div className="space-y-4">
                {viewingInquiryModal.students.map((st, idx) => {
                  ...
                  return (
                    <div key={st.id} className="bg-white p-4 rounded-xl border border-slate-300 space-y-3">
                      ...
                      {evalItem ? (
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                          <div className="bg-white p-2 rounded-lg border border-slate-200">
                            <span className="text-slate-500 block text-[10px]">التحصيل الدراسي:</span>
                            <span className="font-bold text-slate-900">{evalItem.academicLevel}</span>
                          </div>
                          <div className="bg-white p-2 rounded-lg border border-slate-200">
                            <span className="text-slate-500 block text-[10px]">الانضباط الصفي:</span>
                            <span className="font-bold text-slate-900">{evalItem.disciplineLevel}</span>
                          </div>
                          <div className="bg-white p-2 rounded-lg border border-slate-200">
                            <span className="text-slate-500 block text-[10px]">السلوك والمواظبة:</span>
                            <span className="font-bold text-slate-900">{evalItem.behaviorLevel}</span>
                          </div>
                          <div className="bg-white p-2 rounded-lg border border-slate-200">
                            <span className="text-slate-500 block text-[10px]">المشاركة والواجبات:</span>
                            <span className="font-bold text-slate-900">{evalItem.participationLevel}</span>
                          </div>

                          {evalItem.teacherNotes && (
                            <div className="col-span-2 sm:col-span-4 bg-white p-3 rounded-lg border border-slate-200 text-xs space-y-1">
                              <span className="text-slate-500 block text-[10px] font-bold">ملاحظات وتوصيات المعلم:</span>
                              <p className="text-slate-900 leading-relaxed">{evalItem.teacherNotes}</p>
                            </div>
                          )}
                        </div>
                      ) : (
                        <div className="p-3 bg-white border border-slate-200 text-slate-600 rounded-lg text-xs">
              ...
              <div className="pt-6 border-t-2 border-slate-800 grid grid-cols-3 gap-4 text-center text-xs">
                <div className="space-y-1">
                  ...
                  <strong className="text-slate-900 font-bold block">{viewingInquiryModal.teacherName}</strong>
                </div>
                <div className="space-y-1">
                  <span className="text-slate-500 block text-[10px]">الموجه الطلابي:</span>
                  <strong className="text-slate-900 font-bold block">
                    {schoolSignatories.counselorName || "أ. فهد التوجيه"}
                  </strong>
                </div>
                <div className="space-y-1">
                  <span className="text-slate-500 block text-[10px]">مدير المدرسة:</span>
                  <strong className="text-slate-900 font-bold block">
            ...
            <div className="flex items-center justify-between gap-3 pt-2 print:hidden">
              <button
                ...
                className="py-2.5 px-4 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold text-xs flex items-center gap-1.5 cursor-pointer shadow-xs"
              >
                <Printer className="w-4 h-4" />
                <span>طباعة التقرير الرسمي (A4 موفر للحبر)</span>
              </button>
      ...
      {/* ========================================================================= */}
      {/* MODAL: ANTI-BAN SAFE BATCH SENDING PROGRESS */}
      {/* ========================================================================= */}
      {inquiryBatchProgress.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs">
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 sm:p-7 space-y-5 shadow-2xl border border-slate-200">
            {/* Header */}
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm sm:text-base font-black text-slate-900 flex items-center gap-2">
                    <span>إرسال استعلامات المعلمين بنظام درع الحظر</span>
                  </h3>
                  <p className="text-[11px] text-slate-500">
                    فاصل أمان زمني مع تفاوت بشري عشوائي لمنع حظر رقم الواتساب
                  </p>
                </div>
              </div>

              {!inquiryBatchProgress.isRunning && (
                <button
                  type="button"
                  onClick={() => {
                    setInquiryBatchProgress((prev) => ({ ...prev, isOpen: false }));
            ...
            {/* Countdown / Safety Status Indicator */}
            {inquiryBatchProgress.countdownSeconds > 0 ? (
              <div className="bg-amber-50 border border-amber-200 rounded-2xl p-3.5 flex items-center justify-between gap-3 text-amber-900 animate-pulse">
                <div className="flex items-center gap-2.5">
                  <Clock className="w-5 h-5 text-amber-600 shrink-0" />
                  <div>
                    <span className="text-xs font-black block">
                      فاصل الأمان الذكي قيد العد التنازلي: {inquiryBatchProgress.countdownSeconds} ثانية
                    </span>
                    <span className="text-[10px] text-amber-700">
                      تفاوت بشري عشوائي لتجنب الرصد الآلي وحظر الرقم على واتساب
                    </span>
                  </div>
                </div>
                <div className="text-lg font-black font-mono text-amber-700 bg-amber-100 px-3 py-1 rounded-xl">
                  {inquiryBatchProgress.countdownSeconds}s
                </div>
              </div>
            ) : inquiryBatchProgress.isRunning ? (
              <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-3 flex items-center gap-2.5 text-emerald-900">
                ...
                <span className="text-xs font-bold">
                  جاري إرسال الاستعلام للمعلم: {inquiryBatchProgress.currentTeacherName}...
                </span>
              </div>
            ) : inquiryBatchProgress.isCompleted ? (
              <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-3 flex items-center gap-2.5 text-emerald-900">
                <CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0" />
                <span className="text-xs font-bold">
                  اكتملت عملية إرسال الاستعلامات بنجاح تام وبأمان كامل.
                </span>
              </div>
            ) : (
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 flex items-center gap-2.5 text-slate-700">
                <AlertCircle className="w-4 h-4 text-slate-500 shrink-0" />
                <span className="text-xs font-bold">تم إيقاف عملية الإرسال مؤقتاً.</span>
              </div>
            )}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs font-bold text-slate-700">
                ...
                <span>
                  {inquiryBatchProgress.currentIndex} من {inquiryBatchProgress.total} (
                  {Math.round(
                    (inquiryBatchProgress.currentIndex / (inquiryBatchProgress.total || 1)) * 100
                  )}
                  %)
                </span>
              </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-xs">
              <div className="p-2.5 bg-emerald-50 border border-emerald-100 rounded-2xl">
                <span className="text-[10px] text-emerald-600 block font-bold">ناجح</span>
                <strong className="text-base text-emerald-700 font-black">
                  {inquiryBatchProgress.sentCount}
                </strong>
              </div>
              <div className="p-2.5 bg-rose-50 border border-rose-100 rounded-2xl">
                <span className="text-[10px] text-rose-600 block font-bold">فشل</span>
                <strong className="text-base text-rose-700 font-black">
                  {inquiryBatchProgress.failedCount}
                </strong>
              </div>
              <div className="p-2.5 bg-slate-50 border border-slate-100 rounded-2xl">
                <span className="text-[10px] text-slate-500 block font-bold">المتبقي</span>
                <strong className="text-base text-slate-700 font-black">
                  {Math.max(
                    0,
                    inquiryBatchProgress.total -
                      (inquiryBatchProgress.sentCount + inquiryBatchProgress.failedCount)
                  )}
                </strong>
              </div>
            </div>
            ...
            {inquiryBatchProgress.isRunning && (
              <div className="p-3 bg-slate-50 rounded-2xl border border-slate-200 text-xs space-y-1.5">
                <span className="text-[10px] font-bold text-slate-400 block">المعلم الحالي:</span>
                <div className="flex items-center justify-between">
                  <strong className="text-slate-800 font-black text-sm">
                    {inquiryBatchProgress.currentTeacherName}
                  </strong>
                  <span className="text-emerald-700 font-bold bg-emerald-100/70 px-2 py-0.5 rounded-lg text-[10px]">
                    {inquiryBatchProgress.currentSubject}
                  </span>
                </div>
                <div className="flex items-center justify-between text-[11px] text-slate-500">
                  <span>الشعبة: {inquiryBatchProgress.currentSection}</span>
                  <span>الرقم: {inquiryBatchProgress.currentPhone || "غير مسجل"}</span>
                </div>
              </div>
            )}
            ...
            <div className="space-y-1.5">
              ...
              <div className="max-h-36 overflow-y-auto space-y-1.5 p-2 bg-slate-50 rounded-2xl border border-slate-200 text-xs">
                {inquiryBatchProgress.logs.length === 0 ? (
                  <div className="text-center text-slate-400 py-3 text-[11px]">
                    جاري بدء الإرسال وتسجيل الأحداث...
                  </div>
                ) : (
                  inquiryBatchProgress.logs.map((log, idx) => (
                    <div
                      key={idx}
                    ...
                    >
                      <div className="flex items-center gap-1.5 min-w-0">
                        ...
                        <span className="font-bold text-slate-800 truncate">{log.teacherName}</span>
                        <span className="text-slate-400 text-[10px]">({log.subject})</span>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        >
                          {log.status === "success" ? "تم بنجاح" : "تعذر"}
                        </span>
                      </div>
                    </div>
                  ))
              ...
              {inquiryBatchProgress.isRunning ? (
                <>
                  <button
                    type="button"
                    onClick={() => {
                      pauseInquiryBatchRef.current = !pauseInquiryBatchRef.current;
                      setInquiryBatchProgress((prev) => ({
                        ...prev,
                        isPaused: pauseInquiryBatchRef.current,
                      }));
                    }}
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      if (confirm("هل أنت متأكد من رغبتك في إيقاف عملية الإرسال؟")) {
                        abortInquiryBatchRef.current = true;
                        setInquiryBatchProgress((prev) => ({
                          ...prev,
                          isRunning: false,
                        }));
                ...
                <button
                  ...
                  className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-bold text-xs shadow-xs transition-colors cursor-pointer text-center"
                >
                  إغلاق والانتقال لسجل الاستعلامات
                </button>
              )}
            </div>
          </div>
